...
void outputRedirectHandler(char command[], int pipeCount, int outputRedirectionCount, int background, int recursion);

// handling input redirection to take input from a file
void inputRedirectHandler(char command[], int pipeCount, int outputRedirectionCount, int background, int recursion)
{
    char **argu=stringSplitter(command, "<");
    ...
    if(recursion == 1)
        outputRedirectHandler(argu[0], pipeCount, outputRedirectionCount, background, 0);
    else if(pipeCount>0)
...
void outputRedirectHandler(char command[], int pipeCount, int outputRedirectionCount, int background, int recursion)
{
    ...
    if(recursion == 1)
        inputRedirectHandler(argu[0], pipeCount, outputRedirectionCount, background, 0);
    else if(pipeCount>0)
        pipeExec(argu[0], pipeCount);
    else
        simpleExec(argu[0],background);
...
void redirectHandler(char command[], int pipeCount, int outputRedirectionCount, int background)
{   
    char first;
    ...
    if(first=='<')
        outputRedirectHandler(command, pipeCount, outputRedirectionCount, background, 1);
    else
        inputRedirectHandler(command, pipeCount, outputRedirectionCount, background, 1);
}
...
void commandHandler(char command[])
{
    ...
    if(outputRedirectionCount!=0 && inputRedirection!=0)
        redirectHandler(command, pipeCount, outputRedirectionCount, background);
    else if(outputRedirectionCount!=0)
        outputRedirectHandler(command, pipeCount, outputRedirectionCount, background, 0);
    else if(inputRedirection!=0)
        inputRedirectHandler(command, pipeCount, outputRedirectionCount, background, 0);
    else if(pipeCount>0)
